...
import getEJHeaders, { uniqueId } from './ejHeadersSchema';
...
const ExportJob = () => {
  ...
  const editButtonHandler = (val) => {
    navigate(`${routes.CATALOG_EXPORT_JOB}/${val.id}`);
  };
      ...
      <Card className={classes.filters}>
        ...
        <div>
          <Button
            ...
            borderRadius="0"
            onClick={() => navigate(`${routes.CATALOG_EXPORT_JOB_DETAILS}`, { state: { addNew: true } })}
          >
            <span>+</span>
            Add New
          </Button>
        ...
        <Scrollbars className={classes.scrollbars}>
          {
            (isLoading || error) ? <Spinner parent />
              : (
                <Table
                  data={exportJobs?.content}
                  pagination={exportJobs?.pagination}
                  headers={getEJHeaders(editButtonHandler)}
