...
import { StyledProductOfferingsBanner } from '../../../ProductOfferings/StyledProductOffers';

import GoABannerContent from './GoABannerContent/GoABannerContent';

const GoABanner = ({ company }) => {
  const { companyColors } = useTheme();
  const { currentUser } = useGetCurrentUser();

  const companyName = company?.name || currentUser?.organizationName;
  ...
  return (
    <StyledProductOfferingsBanner>
      <StyledFlex direction="row" alignItems="center" justifyContent="space-between">
        <GoABannerContent companyName={companyName} companyColor={companyColor} />
